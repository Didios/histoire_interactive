...
def lire_fichier(file, ligne_vide = False):
    ...
    if fichier_existe(file): # on vérifie que le fichier existe
        import codecs # Ce module permet d'obtenir les caractères accentuées
...
def add_fichier(path, file, contenu = ""):
    ...
    if path != "":
        path = path + "/"
    with codecs.open(path + file, "x", "utf-8") as fichier:
...
"""
ajouter une fonction pour ouvrir des images
"""